...
MammaryGland <- merge(x = `SRR8452054Gene/filtered`, 
                      y = c(`SRR8452055Gene/filtered`, `SRR8452056Gene/filtered`,
                            `SRR8452057Gene/filtered`, `SRR8452058Gene/filtered`, `SRR8452059Gene/filtered`),
                      add.cell.ids = c("Ctrl_1","E2_1","PDBE_1", "Ctrl_2", "E2_2","PDBE_2"), 
...
# MammaryGland$mitoRatio <- PercentageFeatureSet(object = MammaryGland, features = mito.genes)
# MammaryGland$mitoRatio <- MammaryGland@meta.data$mitoRatio / 100
# mito.genes <- rownames(MammaryGland)[grep("^mt-",rownames(MammaryGland))]
# mito.genes <- grep(pattern = "^mt-", x = rownames(x = MammaryGland), value = TRUE)
mouse_mito <- as.tibble(read.csv("/media/hkh/8TB/XUANTHANG/References/Mouse.MitoCarta3.0.csv", header = TRUE))
mito.genes <- as.character(mouse_mito$Symbol)
mito.genes <- mito.genes[mito.genes %in% rownames(MammaryGland)]
MammaryGland$mitoRatio <- Matrix::colSums(MammaryGland[mito.genes,]) / Matrix::colSums(MammaryGland)
...
metadata <- metadata %>% dplyr::rename(seq_folder = orig.ident, 
                                       ...
                                       nGene = nFeature_RNA)
head(metadata)
# Create sample and replication column
metadata$sample <- NA
metadata$sample[which(str_detect(metadata$cells, "^Ctrl_"))] <- "Ctrl"
metadata$sample[which(str_detect(metadata$cells, "^E2_"))] <- "E2"
metadata$sample[which(str_detect(metadata$cells, "^PDBE_"))] <- "PDBE"
metadata$batch <- NA
metadata$batch[which(str_detect(metadata$cells, "^*_1_"))] <- "Rep1"
metadata$batch[which(str_detect(metadata$cells, "^*_2_"))] <- "Rep2"
...
# Visualize the number of cell counts per sample
metadata %>% 
  ggplot(aes(x=sample, fill=batch)) + 
  geom_bar() +
  theme_classic() +
...
metadata %>% 
  ggplot(aes(color=sample, x=nUMI, fill= batch)) + 
...
metadata %>% 
  ggplot(aes(color=sample, x=nGene, fill= batch)) + 
  geom_density(alpha = 0.2) + 
  theme_classic() +
  scale_x_log10() + 
...
metadata %>% 
  ggplot(aes(x=sample, y=log10(nGene), fill=batch)) + 
  geom_boxplot() + 
  theme_classic() +
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1)) +
  theme(plot.title = element_text(hjust=0.5, face="bold")) +
...
metadata %>% 
  ggplot(aes(color=sample, x=mitoRatio, fill=batch)) + 
  geom_density(alpha = 0.2) + 
  scale_x_log10() + 
  theme_classic() +
...
metadata %>%
  ggplot(aes(x=log10GenesPerUMI, color = sample, fill=batch)) +
  geom_density(alpha = 0.2) +
  theme_classic() +
...
head(MammaryGland_filtered@meta.data)
# Visualize QC metrics as a violin plot
